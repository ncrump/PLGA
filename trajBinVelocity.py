...
for i in range(frames):
    ...
    for j in range(Nmol):
        ...
        nz[binNum] += 1
        indx += 1
    aveVxBin.append(vx/nx)
    aveVyBin.append(vy/ny)
    aveVzBin.append(vz/nz)

# get mean velocities per bin vs time for plotting
aveVx,aveVy,aveVz = [],[],[]
vx,vy,vz = np.zeros(frames),np.zeros(frames),np.zeros(frames)
# loop over frames
for i in range(xbin):
    vx,vy,vz = vx*0,vy*0,vz*0
    # loop over bins per frame
...
for i in range(xbin):
    plt.text(bins[i]+0.20*dx,0,str(i+1),fontsize=15,color='b')
plt.savefig('Images/'+str(cnt)+'velocitytime.png')
plt.close()
# plot velocities
for i in range(xbin):
    plt.figure()
    plt.subplot(311)
    plt.plot(t,aveVx[i],'r-')
    plt.ylim(vxmin,vxmax)
    plt.grid()
    plt.ylabel(r'$ \overline{Vx}$',fontsize=fontsize)
    plt.hlines(0,0,t[-1],'k',linestyle='dashed')
    plt.subplot(312)
    plt.plot(t,aveVy[i],'g-')
    plt.ylim(vymin,vymax)
    plt.grid()
    plt.ylabel(r'$ \overline{Vy}$',fontsize=fontsize)
    plt.hlines(0,0,t[-1],'k',linestyle='dashed')
    plt.subplot(313)
    plt.plot(t,aveVz[i],'b-')
    plt.ylim(vzmin,vzmax)
    plt.grid()
    plt.ylabel(r'$ \overline{Vz}$',fontsize=fontsize)
    plt.xlabel('$time$',fontsize=fontsize)
    plt.hlines(0,0,t[-1],'k',linestyle='dashed')
    plt.annotate('Bin='+'%3i'%(i+1),fontsize=fontsize,xy=(0.78,0.92),xycoords='figure fraction')
    plt.annotate('$(nm/ps)$',fontsize=16,xy=(0.05,0.93),xycoords='figure fraction')
    plt.annotate('$(ns)$',fontsize=16,xy=(0.84,0.025),xycoords='figure fraction')
    plt.savefig('Images/'+str(cnt+1)+'velocitytime.png')
    plt.close()
    cnt += 1

print cnt+1,'images saved'
print 'done !\n'
plt.ion()